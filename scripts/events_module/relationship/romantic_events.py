...
class RomanticEvents:
    ...
    def check_if_breakup(cat_from, cat_to):
        ...
        if (
            (not cat_from.status.alive_in_player_clan and (cat_from.status.is_lost(CatGroup.PLAYER_CLAN) or cat_from.status.is_exiled(CatGroup.PLAYER_CLAN)))
            or not (cat_to.status.alive_in_player_clan and (cat_to.status.is_lost(CatGroup.PLAYER_CLAN) or cat_to.status.is_exiled(CatGroup.PLAYER_CLAN)))
        ):
            return False
