    ...
    def draw_sprite(phenotype, cat_sprite, somatic = False):
        ...
        def GenSprite(phenotype, sprite_age, merle=False):
            ...
            def AddStripes(whichmain, whichcolour, whichbase, coloursurface=None):
                stripebase = pygame.Surface(
                    (sprites.size, sprites.size), pygame.HWSURFACE | pygame.SRCALPHA)
                if (('ec' in phenotype.ext or (phenotype.ext[0] == 'ea' and ((sprite_age > 7 and phenotype.ext[0] != "a") or sprite_age > 19))) and 'Eg' not in phenotype.ext and not ('red' in whichcolour or 'cream' in whichcolour or 'honey' in whichcolour or 'ivory' in whichcolour or 'snow' in whichcolour or 'apricot' in whichcolour)):
                    stripebase = CreateStripes(
